/* sound.js — penjana bunyi ringkas untuk kesan bunyi (tiada fail audio luaran).
   Pelayar memerlukan gerak isyarat pengguna sebelum audio dimainkan — ensureCtx() menguruskannya. */
(function () {
  "use strict";
  let ctx = null;
  let masterGain = null;
  ...
  let muted = false;
  try { muted = localStorage.getItem("bq_muted") === "1"; } catch (e) { }
  ...
  window.SOUND = {
    ...
    toggleMute() {
      muted = !muted;
      try { localStorage.setItem("bq_muted", muted ? "1" : "0"); } catch (e) { }
